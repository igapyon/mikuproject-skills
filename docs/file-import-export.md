...
主要な入口は `globalThis.__mikuprojectCoreApi` です。
...
- XML テキストを受け取り、`msProject.importFromXml` で `ProjectModel` に変換します
- 必要なら validate し、`workbookJson.exportDocument` で `mikuproject_workbook_json` に揃えます
- file workflow から AI workflow へ入る入口として使えます

### export

- current state を `ProjectModel` に揃え、`msProject.exportToXml` で XML テキストを生成します
...
- `ProjectModel` の場合は `workbookJson.exportDocument` で変換します
...
binary を直接扱う場合は `importExternal({ source: { format: "xlsx", bytes }, ... })` を使えます。
