...
(() => {
  ...
  function applyAddTaskOperation(
    ...
    const normalizedStartCandidate = normalizeOptionalPatchedTaskDate(
      operation.planned_start,
      "start",
      { uid, name } as TaskModel,
      project
    );
    ...
    const normalizedFinishCandidate = normalizeOptionalPatchedTaskDate(
    ...
    changes.push({
      ...
      field: "position",
      before: undefined,
      after: nextPositionMap.get(uid) ?? nextIndex
    });
  ...
  function applyAddAssignmentOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
    ...
    const taskUid = String((operation as PatchOperation & { task_uid?: string }).task_uid || "").trim();
    if (!taskUid) {
    ...
    const resourceUid = String((operation as PatchOperation & { resource_uid?: string }).resource_uid || "").trim();
    if (!resourceUid) {
    ...
    warnUnsupportedPatchKeysForScope(
      operation,
      new Set(["op", "uid", "task_uid", "resource_uid", "start", "finish", "units", "work", "percent_work_complete"]),
      "add_assignment",
      "assignments",
      uid,
      uid,
    ...
    if (operation.percent_work_complete !== undefined) {
      const percentWorkComplete = (operation as PatchOperation & { percent_work_complete?: unknown }).percent_work_complete;
    ...
    const addedAssignment: AssignmentModel = {
      uid,
      taskUid,
      resourceUid,
      start: normalizedStart,
      ...
      work,
      percentWorkComplete: (operation as PatchOperation & { percent_work_complete?: number }).percent_work_complete,
      extendedAttributes: [],
      baselines: [],
      timephasedData: []
    };
    ...
    const percentWorkComplete = (operation as PatchOperation & { percent_work_complete?: number }).percent_work_complete;
  ...
  function applyAddResourceOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
    ...
    warnUnsupportedPatchKeysForScope(
      operation,
      new Set(["op", "uid", "name", "initials", "group", "calendar_uid", "max_units", "standard_rate", "overtime_rate", "cost_per_use", "percent_work_complete"]),
      "add_resource",
      "resources",
      uid,
      name,
    ...
    let nextCalendarUid: string | undefined;
    ...
    if ((operation as PatchOperation & { cost_per_use?: unknown }).cost_per_use !== undefined) {
      const costPerUse = (operation as PatchOperation & { cost_per_use?: unknown }).cost_per_use;
    ...
    if ((operation as PatchOperation & { percent_work_complete?: unknown }).percent_work_complete !== undefined) {
      const percentWorkComplete = (operation as PatchOperation & { percent_work_complete?: unknown }).percent_work_complete;
      if (typeof percentWorkComplete !== "number" || !Number.isFinite(percentWorkComplete) || percentWorkComplete < 0 || percentWorkComplete > 100) {
    ...
    if ((operation as PatchOperation & { standard_rate?: unknown }).standard_rate !== undefined && typeof (operation as PatchOperation & { standard_rate?: unknown }).standard_rate !== "string") {
      warnings.push({ message: `add_resource.standard_rate は文字列が必要です: ${uid}`, scope: "resources", uid, label: name });
      return undefined;
    }
    if ((operation as PatchOperation & { overtime_rate?: unknown }).overtime_rate !== undefined && typeof (operation as PatchOperation & { overtime_rate?: unknown }).overtime_rate !== "string") {
      warnings.push({ message: `add_resource.overtime_rate は文字列が必要です: ${uid}`, scope: "resources", uid, label: name });
      return undefined;
    }

    const addedResource: ResourceModel = {
      uid,
      id: "",
      name,
      initials: operation.initials?.trim() || undefined,
      group: operation.group?.trim() || undefined,
      maxUnits: operation.max_units,
      calendarUID: nextCalendarUid,
      standardRate: (operation as PatchOperation & { standard_rate?: string }).standard_rate?.trim() || undefined,
      overtimeRate: (operation as PatchOperation & { overtime_rate?: string }).overtime_rate?.trim() || undefined,
      costPerUse: (operation as PatchOperation & { cost_per_use?: number }).cost_per_use,
      percentWorkComplete: (operation as PatchOperation & { percent_work_complete?: number }).percent_work_complete,
      extendedAttributes: [],
      baselines: [],
      timephasedData: []
  ...
  function applyAddCalendarOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
    ...
    warnUnsupportedPatchKeysForScope(
      operation,
      new Set(["op", "uid", "name", "is_base_calendar", "base_calendar_uid"]),
      "add_calendar",
      "calendars",
      uid,
      name,
    ...
    if (operation.is_base_calendar !== undefined && typeof (operation as PatchOperation & { is_base_calendar?: unknown }).is_base_calendar !== "boolean") {
      warnings.push({ message: `add_calendar.is_base_calendar は boolean が必要です: ${uid}`, scope: "calendars", uid, label: name });
      return undefined;
    }
    let nextBaseCalendarUid: string | undefined;
    if ((operation as PatchOperation & { base_calendar_uid?: unknown }).base_calendar_uid !== undefined) {
      const rawBaseCalendarUid = (operation as PatchOperation & { base_calendar_uid?: unknown }).base_calendar_uid;
    ...
    const addedCalendar: CalendarModel = {
      uid,
      name,
      isBaseCalendar: Boolean((operation as PatchOperation & { is_base_calendar?: boolean }).is_base_calendar),
  ...
  function applyDeleteResourceOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
  ...
  function applyDeleteCalendarOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
  ...
  function applyDeleteAssignmentOperation(
    operation: PatchOperation,
    model: ProjectModel,
    changes: ImportChange[],
    warnings: PatchWarning[],
  ...
  function warnUnsupportedPatchKeys(
    operation: PatchOperation,
  ...
  function warnUnsupportedPatchKeysForScope(
    operation: PatchOperation,
    allowedKeys: Set<string>,
    opName: string,
    scope: "project" | "tasks" | "resources" | "assignments" | "calendars",
  ...
  function normalizeOptionalPatchedTaskDate(
    ...
    return normalizePatchedTaskDate(rawValue, kind, task as TaskModel, project);
  }
  ...
  function normalizeDurationText(duration: string | undefined): string | undefined {
    const text = String(duration || "").trim();
    return text || undefined;
  }

  function isValidDurationText(duration: string): boolean {
    const text = String(duration || "").trim();
    if (!text) {
      return false;
    }
  ...
  function formatPredecessor(predecessor: PredecessorModel): string {
  ...
  function formatRequestedDependencyRelation(
    fromUid: string,
    toUid: string,
  ...
  function normalizePatchedPlainDateTime(
    value: unknown,
    kind: "start" | "finish",
    project: ProjectInfo
  ): string | undefined {
