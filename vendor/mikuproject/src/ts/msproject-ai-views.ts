...
(() => {
  ...
  function buildDefaultRules(scope: "project_overview_view" | "phase_detail_view" | "task_edit_view") {
  ...
  const aiViews = {
    ...
    exportProjectOverviewView(model: ProjectModel) {
      ...
      const topLevelDependencyMap = new Map<string, { from_uid: string; to_uid: string; type: string }>();
    ...
    exportTaskEditView(model: ProjectModel, requestedTaskUid?: string) {
      ...
      const predecessors = targetTask.predecessors.map((item) => ({
        ...
        name: taskByUid.get(item.predecessorUid)?.name || item.predecessorUid,
        type: formatDependencyType(item.type),
        lag: item.linkLag || "PT0H0M0S",
        lag_hours: parseDurationHours(item.linkLag || "PT0H0M0S") ?? 0
      }));
      const successors = tasks.flatMap((task) =>
        task.predecessors
      ...
      const assignments = model.assignments
        ...
        .map((assignment) => {
          ...
          return {
            ...
            resource_name: resource?.name || assignment.resourceUid,
      ...
      return {
        ...
        target_task: {
          ...
          position: positionMap.get(targetTask.uid) ?? 0,
        ...
        parent_task: parentTask ? {
          ...
          position: positionMap.get(parentTask.uid) ?? 0
        } : null,
  ...
  (globalThis as typeof globalThis & {
    __mikuprojectMsprojectAiViews?: typeof aiViews;
  }).__mikuprojectMsprojectAiViews = aiViews;
})();
