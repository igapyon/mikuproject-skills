...
(() => {
  const mainUi = {
    getTabButtons(doc: Document): HTMLButtonElement[] {
      return Array.from(doc.querySelectorAll<HTMLButtonElement>(".md-top-tab[data-tab]"));
    },

    getTabPanels(doc: Document): HTMLElement[] {
      return Array.from(doc.querySelectorAll<HTMLElement>(".md-tab-panel[data-tab-panel]"));
    },
    ...
    updateXmlSaveState(doc: Document, input: {
      isDirty: boolean;
  ...
  (globalThis as typeof globalThis & {
    __mikuprojectMainUi?: typeof mainUi;
  }).__mikuprojectMainUi = mainUi;
})();
