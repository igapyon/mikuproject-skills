 ...
 */
(() => {
  type MainIoArchiveEntry = { name: string; data: Uint8Array };

  type MainIoArchiveDeps = {
    model: ProjectModel;
  ...
  const mainIo = {
    buildOutputArchiveEntries(deps: MainIoArchiveDeps): MainIoArchiveEntry[] {
      ...
      const projectOverview = deps.exportProjectOverviewView(deps.model) as { phases?: Array<{ uid?: string }> };
      ...
      entries.push(
        { name: "mikuproject-project-overview-view.editjson", data: deps.encodeUtf8(`${JSON.stringify(projectOverview, null, 2)}\n`) },
    ...
    detectImportKind(input: {
      ...
      detectJsonDocumentKind: (documentLike: unknown) => MainIoImportKind | undefined;
    }): MainIoImportKind {
  ...
  (globalThis as typeof globalThis & {
    __mikuprojectMainIo?: typeof mainIo;
  }).__mikuprojectMainIo = mainIo;
})();
